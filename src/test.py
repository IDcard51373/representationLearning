...
for i in range(len(Y_test)):
    kits.to_img(Y_lossed[i]).save("..\\out\\" + str(i) + "_loss.pgm")
    kits.to_img(Y_test[i]).save("..\\out\\" + str(i) + "_ori.pgm")
    ...
    kits.to_img(rec_img).save("..\\out\\" + str(i) + "_rec.pgm")
# 5. 计算重建人脸图像与原始图像的平均误差。并输出图像
for i in range(len(Y_test)):
    print(str(i)+"次实验的psnr为："+str(kits.psnr(Y_test[i], Y_rec[i])))
"""
note:
要切片！否则归一化之后就是0了
